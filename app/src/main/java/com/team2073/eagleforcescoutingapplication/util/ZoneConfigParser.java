...
public class ZoneConfigParser {
    ...
    public static void parseAndRestoreZonesRelative(ZoneDrawingView zoneDrawingView, String config) {
        ...
        try {
            ...
            // --- FIX START: Use the boundaries FROM THE CONFIGURATION STRING ---
            if (parts.length < 2 || parts[1].isEmpty()) {
                android.util.Log.e("ZoneConfigParser", "Configuration string is missing boundary data.");
                return;
            }

            String[] boundaryParts = parts[1].split(",");
            if (boundaryParts.length < 4) {
                android.util.Log.e("ZoneConfigParser", "Invalid boundary data in configuration.");
                return;
            }

            float fieldLeft = Float.parseFloat(boundaryParts[0]);
            float fieldTop = Float.parseFloat(boundaryParts[1]);
            float fieldRight = Float.parseFloat(boundaryParts[2]);
            float fieldBottom = Float.parseFloat(boundaryParts[3]);
